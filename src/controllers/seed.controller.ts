...
export const seedDatabase = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: 'Database seeded successfully with clean, luxury fashion data and verified Unsplash images!',
      seededProductsCount: createdProducts.length,
      seededCollectionsCount: collectionsData.length
    });
  } catch (error) {
